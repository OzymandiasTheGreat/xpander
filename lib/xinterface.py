...
class Interface(object):
	...
	def _layout_watcher(self):
		...
		while True:
			self.xkb_switch = subprocess.Popen(
				['xkb-switch', '-w', '-p'],
				stdout=subprocess.PIPE,
				universal_newlines=True)
			output = self.xkb_switch.stdout.readline().strip('\n')
			self.current_layout = self.layout_split.match(output).groups()
			self.switch_layout(self.current_layout)

	def switch_layout(self, layout):

		transient_layouts = sorted(
		...
		try:
			...
			print("Failed closing display socket")
		self.local_display = display.Display()
		self.root_window = self.local_display.screen().root
		self.update_active_window()
	...
	def handle_key_event(self, type, keycode, state, window_class, window_title):
	...
	def keysym_to_keycode(self, keysym):
		...
		try:
			keycode, index = list(self.static_display.keysym_to_keycodes(keysym))[0]
		except IndexError:
			keycode, index = (0, 0)
		state = 0
		if index == 1:
